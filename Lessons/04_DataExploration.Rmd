...
```{r}
# Working directory should be set to the parent folder for the Environmental Data Analytics Course, i.e., the folder that houses the R Project file. Running getwd() will let you know which file path you ar working from
...
```{r, message = FALSE}
# Load package, don't print this on the pdf 
library(tidyverse)
```
...
# Relative file path (friendly for users regardless of machine) preferred method 
...
> ANSWER: The file is in the Data folder in a parallel file with metadata. This is best practice because it is easy to find within a parallel source. The data itself contains little useful info, column names are not useful without context. 

```{r}
View(USGS.flow.data)
# Viewung the R object we created with <-
...
str(USGS.flow.data) #numberic returns NA values if there is blank, character or factor blank
dim(USGS.flow.data) #missed this - dimensions  
length(USGS.flow.data) #number of columns 
nrow(USGS.flow.data) #number of rows 

head(USGS.flow.data) #default is first 6 rows of the data
head(USGS.flow.data, 10) # pulls the first 10 rows of the data 
tail(USGS.flow.data, 5) # pulls the last five rows of the data 
USGS.flow.data[30000:30005, c(3, 8, 14)] #rows 30000 to 30005, missed the rest 

class(USGS.flow.data$datetime) #why do you copy and paste this as opposed to running it? how do you know to do this?
...
> Answer: if they were numeric, they appear as Na, if they are factor or character they come back as 1
...
Sometimes knowing NA exists is informative, presence/absence data. Sometimes can only run whole set of data, if you remove the NAs. 

> Answer: 

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data)
dim(USGS.flow.data) #unomitted NA
dim(USGS.flow.data.complete) #ommitted NAs
...
#format(today, format = "%b")
#calling up object and the specific column, assignment operator is formatting it something new, as date is the function 
...
```{r}
#whaterformat the data is in, not what R has 
#formatfunction, date time is in character format  
...
#row names makes sure that there is not a column row number in the new file
```
