...
Setting - "defining" telling the system what it already knows 
st_set_crs() R will give you a warning to define it 
Projecting - "changing" into a new system 
...
#plot window may take awhile with large amounts of data - so you can save as a pdf 
#using here function instead of relative file paths 
```
...
```{r activity.here}
##we've been utilizing relative file paths before 
# test out here
here()
##relative file path from which we're navigating 
...
##going inside the folders "data"-> "tabular data" ->then file within that folder "coterm_basinid.txt" similar to before, 
##running the code just generates the file path 
...
##political boundaries, habitat cover, wildlife observation
...
The *sp* package is the first iteration of an R package created to handle vector spatial data. Given the likelihood that you'll come across old code, it's helpful to be familiar with both the older sp package and it's newer version, the sf package. 1995. SF built from SP. SP first iteration of SF.  
...
* New in 2016 and growing rapidly, ##update your package often 
...
#have to distort 3D surface to be modeled by 2D 
...
## May need to find diffreent projections if working outside of US and Europe, or working in Alaska. 
...
The data we'll use in this lesson comes from the United States Geologic Survey (USGS). Specifically, the dataset is called the Geospatial Attributes of Gages for Evaluating Steamflow, version 2 (GAGES-II) available [here](https://water.usgs.gov/GIS/metadata/usgswrd/XML/gagesII_Sept2011.xml). GAGES-II provides geospatial data and classifications of 9,322 stream gages that are maintained by the USGS. It includes: (1) watershed boundaries by region (spatial data), (2) USGS stream gage locations (spatial data), (3) watershed characteristics (tabular data). Note that the term "watershed" is used interchangeably with the word "basin". ## and catchment 

We'll also being using Southeast state boundary data from the United States Census Bureau.
##maps package also helpful 
...
##reading in a shape package
##only need to read in shp file, but need the other 6 different type of files associated with it 
...
#choosing climate data here 
##try out read.table and read.csv to make sure you're getting the correctly formatted data 
...
##sf and data frames 
col(basins_nf_seplains_raw)
col_name(basins_nf_seplains_raw)
...
While not in base, you can also use glimpse() from the tibble (tidyverse) package to look at basin_nf_seplains_raw. ## used for spatial data
...
##will always match up, so if one is missing and other is not, can use setting
...
##can do this in the same line of code 
##just like setting the column names 
...
# setting projections ## sub in the name for coordinate reference systems 
...
# convert to sf object using st_as_sf function 
...
##latitude and longitude no longer there, but we have a geometry column 
st_crs(gages_tabular_as_sf)
##asking for coordinate reference system 
...
ggplot() +
  ...
  theme_bw()
dev.off()
# seems to work just fine! ## this didn't work for me 
```
...
## just geometry column, dropped tabular data 
...
# geometry data is dropped, just tabular data remains 
...
Clip gages that are in North Carolina, political 
...
Geometric operations - produces a spatial data set 

Geometric confirmatin - true, false 
...
##check cheatsheet for other tools 
...
##drops the attribute data 
##sf object has a data frame and a geopgraphy 
# check
# add your code here
head(va_state_bounds_geom)
...
va_basins_nf_seplains <- basins_nf_seplains %>%
  st_intersection(va_state_bound_geom)

##cutting out state basins in virginia 

# check
head(va_basins_nf_seplains)
...
##distance related to the units of the dataset 
# add your code here
va_area <- st_area(va_state_bound_geom) # find the area of Virginia
va_centroid <- st_centroid(va_state_bound_geom)
va_gages_buffer_100m <- st_buffer(va_gages, dist = 100) # make a buiffer around the stream gages in Virginia 
...
ggplot() + 
    ...
    geom_sf(data = va_basins_nf_seplains, fill = "blue", alpha = 0.25) + 
    geom_sf(data = va_gages, color = "black")
dev.off()

## can't plot this in R studio with multiple data frames without crashing - need to use dev off and pdf (here)
```
...
##Contextual map, allow us to create images in the background" 
...
nc_bbox <- nc_state_bounds_geom %>%
  ...
  ## translating back into longitude and latitude 
  st_bbox()
##st_bbox() bounding box edges coordinates x y values for boundaries of the buffer 
nc_bbox # check

# fix bounding box columns so they match what is needed for the 
##ggmap::get_map() function
...
dev.off()
##ggmap knows to treat data like a base map 
...
# check
st_crs(fl_state_bounds_geom_wsg84)

# plot without basemap
pdf(here("outputs", "fl_without_basemap.pdf"), width = 11, height = 8.5)
ggplot() +
  geom_sf(data = fl_state_bounds_geom_wsg84, fill = NA, lwd = 2) +
  theme_bw()
dev.off()

# plot with basemap
# windows version?
pdf(here("outputs", "fl_with_basemap.pdf"), width = 11, height = 8.5)
ggmap(fl_basemap) +
  geom_sf(data = fl_state_bounds_geom_wsg84, fill = NA, lwd = 1, inherit.aes = FALSE)
dev.off()
```
...
##joined by stationid 
## use mutate to change the class of one of the STAID 
...
#Saving shape to give it to someone or use in another applicaiton 
## creates four different files, you have to move them all together, to read the shp file 
```
