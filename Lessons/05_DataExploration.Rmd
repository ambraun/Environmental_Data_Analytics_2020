...
#specificy width of the bin, 10 units wide, generates less bins because they are wider 
ggplot(USGS.flow.data) +
...
#c represents concatanate, plus in each line rather than a comment, specific syntax for gg plot to add another line 
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# looking at gage height 
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean))

# formatting code to show up as an object 
...
```{r, fig.height = 3, fig.width = 4}
#first line - for one column of data set, second line specifying a different color than the first line, lty = line type width, frequency polygon = line 
ggplot(USGS.flow.data) +
...
# more data in a certain section - violin is wider, can draw quartiles lines in the violin, can also do scale = "count" at the end 
ggplot(USGS.flow.data) +
