...
#intercept represents paul lake at zero meters depth, 284 micrograms per liter, Peter lake is significantly different from Paul Lake., no difference in the actual formulation, (looking at main effects +)

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)

## beta values are slopes, interactions effects assume different slopes (looking at interactioneffects *)
##Adjusted R-Squared shows the percent 
...
TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) + 
  ...
  #scale_color_brwery(palette = "Dark2")
  color_color_manual(values = c("darkblue", "lightblue"))+
  xlim(0, 10)
print(TNplot)

##can manipulate axis in ggplot to manipulate the tick 
```
...
The coefficients of a hierarchical model are divided into two categories: **fixed effects** and **random effects.** A **fixed effect** is a factor whose levels are experimentally determined or whose interest lies in the effects of each level (e.g., covariates, treatments, interactions). A **random effect** is a factor whose levels are sampled from a larger population, or whose interest lies in the variation among them rather than the specific effect of each level. In choosing whether you are dealing with a fixed or a random effect, consider the following questions: 
  ...
  interested in depth and lake name for nitrogen - if not it would be a random effect 

  + Have you included all possible levels in the study?
  extrapolate beyond my factors level 
  
  + Do you have interest in the variance among levels?
  if not - random effect 
  
  + Do you have interest in generalizing to factor levels that you did not study?
  - generalizing to the entire population, when you only studied 5 people 
...
Space is the variable of interest, interested to know if lake temperature have changed over the past 30 yeras. Not intersted in knowing how specific lakes change, how lakes in the general area have responded to climate change. Use info in variability to characterize the variability. 
...
NTL.summertemp <- 
  ...
  #filter for Julian days in July and surface measurements - trying to find similar temp measurements, what?   
  filter(daynum > 181 & daynum < 213 & depth == 0 ) %>% ## filtering for july and surface depth 
  #code won't work if there are NAs
  na.exclude()

NTLtemps <- ggplot(NTL.summertemp, aes(x = sampledate, y = temperature_C, color = lakename)) +
  geom_point() +
...
##variablility across lakes as the variability of our model 
# random = 1, grouped by lake name for random effectca, use other numbers but not in this case 

##p value is low for intercept and year - rounded in this case 
## Random effect is defined as 
...
##gls gives slightly different output summary than glm, but does the same thing, 

##have run fixed effects model and random effects model - now running to see if we have a difference between the models 

anova(TempTest.mixed, TempTest.fixed)
# The lower the AIC, the better model fit ?
# The p-value tells us whether those models have a significantly different fit
## lower AIC is for the fixed model 
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  geom_point() +
  scale_color_viridis_d() +
  ##coefficient of the model for slope and intercept, slope represents, 
...
## black is mixed and blue is fixed 
